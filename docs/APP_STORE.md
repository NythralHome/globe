...
- `app/Scripts/package-app-store.sh` builds `app/.build/app-store/Globe-0.1.0-22-mas.pkg`.
...
```sh
cd app
GLOBE_DISTRIBUTION=app-store GLOBE_VERSION=0.1.0 GLOBE_BUILD=22 Scripts/build-app-bundle.sh
```
...
The App Store package uses `CFBundleShortVersionString=0.1.0` and `CFBundleVersion=22`. Keep beta labels in App Store/TestFlight metadata, not in `CFBundleShortVersionString`. The generated `Info.plist` includes `ITSAppUsesNonExemptEncryption=false` so App Store Connect can detect export compliance automatically.

Expected package path:

```text
app/.build/app-store/Globe-0.1.0-22-mas.pkg
```
