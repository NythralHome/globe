...
- `app/Scripts/package-app-store.sh` builds `app/.build/app-store/Globe-0.1.0-36-mas.pkg`.
...
```sh
cd app
GLOBE_DISTRIBUTION=app-store GLOBE_VERSION=0.1.0 GLOBE_BUILD=36 Scripts/build-app-bundle.sh
```
...
```text
app/.build/app-store/Globe-0.1.0-36-mas.pkg
```
